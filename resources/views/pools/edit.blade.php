<x-layouts.app>
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">{{ __('Edit Pool') }}</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-1">{{ $pool->pool_name }}</p>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden max-w-3xl">
        <form action="{{ route('pools.update', $pool) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            <div class="space-y-6">
                <x-forms.input label="Pool Name" name="pool_name" type="text" value="{{ old('pool_name', $pool->pool_name) }}" required />
                ...
                <div>
                    ...
                    <textarea name="address" rows="3"
                        class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">{{ old('address', $pool->address) }}</textarea>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <x-forms.input label="Latitude (lat)" name="lat" type="number" step="0.00000001"
                            value="{{ old('lat', $pool->lat) }}" />
                    </div>
                    <div>
                        <x-forms.input label="Longitude (lng)" name="lng" type="number" step="0.00000001"
                            value="{{ old('lng', $pool->lng) }}" />
                    </div>
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3 mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                ...
                <x-button type="primary">{{ __('Update Pool') }}</x-button>
            </div>
        </form>
    </div>
</x-layouts.app>
