<x-layouts.app>
    ...
    <div class="mb-6 flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">{{ __('View Position') }}</h1>
            <p class="text-gray-600 dark:text-gray-400 mt-1">{{ __('Position details') }}</p>
        </div>
        <div class="flex gap-2">
            @if(auth()->user()->hasPermission('edit-positions'))
                <a href="{{ route('positions.edit', $position) }}">
                    <x-button type="primary">{{ __('Edit Position') }}</x-button>
                </a>
            @endif
            <a href="{{ route('positions.index') }}">
        ...
        <div class="p-6">
            <div class="max-w-2xl">
                ...
                <div class="mb-6">
                    ...
                    <div class="text-gray-900 dark:text-gray-100">
                        {{ $position->updated_at->format('M d, Y H:i') }}
                    </div>
                </div>
            </div>
        </div>
