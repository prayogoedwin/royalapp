<x-layouts.app>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <!-- Notification Alerts -->
        @if($unit->isPajakDueSoon() || $unit->isPlatDueSoon() || $unit->isKirDueSoon())
        <div class="p-6 border-b border-gray-200 dark:border-gray-700 space-y-3">
            ...
            @if($unit->isKirDueSoon())
            <div class="flex items-start p-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg">
                <svg class="w-5 h-5 text-yellow-600 dark:text-yellow-400 mt-0.5 mr-3 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
                </svg>
                <div>
                    <h3 class="text-sm font-medium text-yellow-800 dark:text-yellow-200">KIR Segera Jatuh Tempo</h3>
                    <p class="mt-1 text-sm text-yellow-700 dark:text-yellow-300">
                        {{ $unit->tgl_kir_berikutnya->format('d M Y') }} 
                        ({{ abs($unit->daysUntilKirRenewal()) }} hari lagi)
                    </p>
                </div>
            </div>
            @endif
        ...
        <div class="p-6">
            <dl class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">{{ __('KIR Terakhir') }}</dt>
                    <dd class="text-base text-gray-900 dark:text-gray-100">{{ $unit->tgl_kir_terakhir?->format('d M Y') ?? '-' }}</dd>
                </div>

                <div>
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1">{{ __('KIR Berikutnya') }}</dt>
                    <dd class="text-base text-gray-900 dark:text-gray-100">{{ $unit->tgl_kir_berikutnya?->format('d M Y') ?? '-' }}</dd>
                </div>
