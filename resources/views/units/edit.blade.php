<x-layouts.app>
    <div class="mb-6 flex items-center text-sm">
        ...
        <span class="text-gray-500 dark:text-gray-400">{{ __('Edit Unit') }}</span>
    </div>

    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">{{ __('Edit Unit') }}</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-1">{{ __('Update unit information') }}</p>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <form action="{{ route('units.update', $unit) }}" method="POST" class="p-6">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="number" name="tahun_pembelian" min="1900" max="{{ date('Y') + 1 }}" 
                        value="{{ old('tahun_pembelian', $unit->tahun_pembelian) }}" 
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <x-forms.input 
                            ...
                            type="date" 
                            value="{{ old('tgl_perpanjangan_pajak', $unit->tgl_perpanjangan_pajak?->format('Y-m-d')) }}"
                        />
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <x-forms.input 
                            ...
                            name="tgl_ganti_plat" 
                            type="date" 
                            value="{{ old('tgl_ganti_plat', $unit->tgl_ganti_plat?->format('Y-m-d')) }}"
                        />
                    </div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">{{ old('description', $unit->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex justify-end gap-3 mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                ...
                <x-button type="primary">{{ __('Update Unit') }}</x-button>
            </div>
        </form>
    </div>
</x-layouts.app>
