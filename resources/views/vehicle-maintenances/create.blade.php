<x-layouts.app>
    @php
        $identificationInitial = collect(old('identification_pic_ids', []))->map(fn($id) => ['id' => (int) $id, 'name' => 'Employee #'.$id])->values();
        $repairInitial = collect(old('repair_pic_ids', []))->map(fn($id) => ['id' => (int) $id, 'name' => 'Employee #'.$id])->values();
    @endphp
    <div x-data="vehiclePicForm({ identificationInitial: @js($identificationInitial), repairInitial: @js($repairInitial) })">
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100">Create Vehicle Maintenance</h1>
    </div>

    <form method="POST" action="{{ route('vehicle-maintenances.store') }}" class="space-y-6">
        @csrf
                ...
                <div>
                    ...
                    <select name="maintenance_type" required class="w-full px-3 py-2 border rounded-md bg-white dark:bg-gray-700">
                        @foreach(\App\Models\VehicleMaintenance::TYPE_OPTIONS as $value => $label)
                            <option value="{{ $value }}" @selected(old('maintenance_type') === $value)>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="unit_id" required class="w-full px-3 py-2 border rounded-md bg-white dark:bg-gray-700">
                        ...
                        @foreach($units as $unit)
                            <option value="{{ $unit->id }}" @selected((string) old('unit_id') === (string) $unit->id)>{{ $unit->code }} - {{ $unit->division?->nama }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="order_status_id" required class="w-full px-3 py-2 border rounded-md bg-white dark:bg-gray-700">
                        @foreach($statuses as $status)
                            <option value="{{ $status->id }}" @selected((string) old('order_status_id') === (string) $status->id)>{{ $status->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <x-forms.input label="ODO / Kilometer Identifikasi" name="odo_identification" type="number" step="0.01" value="{{ old('odo_identification') }}" />
                <x-forms.input label="Waktu Teridentifikasi" name="identified_at" type="datetime-local" value="{{ old('identified_at') }}" />
            </div>
            <div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <x-forms.input label="ODO / Kilometer Perbaikan" name="odo_repair" type="number" step="0.01" value="{{ old('odo_repair') }}" />
                <x-forms.input label="Waktu Perbaikan" name="repaired_at" type="datetime-local" value="{{ old('repaired_at') }}" />
            </div>
            <div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            ...
            <div id="cost-list" class="space-y-3"></div>
        </div>
