<x-layouts.app>
    ...
    <div class="mb-6">
        ...
        <p class="text-gray-600 dark:text-gray-400 mt-1">{{ __('Update employee and user account information') }}</p>
    </div>
            ...
            <form action="{{ route('employees.update', $employee) }}" method="POST" class="max-w-4xl">
                ...
                <div class="mb-8 pb-6 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <x-forms.input label="Name" name="name" type="text" value="{{ old('name', $employee->user->name ?? '') }}" required />
                        </div>
                        
                        <div>
                            <x-forms.input label="Email" name="email" type="email" value="{{ old('email', $employee->user->email ?? '') }}" required />
                        </div>
                        
                        <div>
                            <x-forms.input label="Password (leave blank to keep current)" name="password" type="password" />
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('Leave blank if you don\'t want to change the password') }}</p>
                        </div>
                ...
                <div class="mb-8 pb-6 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <x-forms.input label="NIK" name="nik" type="text" value="{{ old('nik', $employee->nik) }}" required />
                        </div>
                        
                        <div>
                            <x-forms.input label="Full Name" name="full_name" type="text" value="{{ old('full_name', $employee->full_name) }}" required />
                        </div>
                ...
                <div class="mb-8 pb-6 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <x-forms.input label="Phone" name="phone" type="text" value="{{ old('phone', $employee->phone) }}" />
                        </div>
                        
                        <div>
                            <x-forms.input label="Birth Date" name="birth_date" type="date" value="{{ old('birth_date', $employee->birth_date?->format('Y-m-d')) }}" />
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="address" rows="3"
                                class="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">{{ old('address', $employee->address) }}</textarea>
                            @error('address')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="mb-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <x-forms.input label="Join Date" name="join_date" type="date" value="{{ old('join_date', $employee->join_date?->format('Y-m-d')) }}" required />
                        </div>
                        
                        <div>
                            <x-forms.input label="Resign Date" name="resign_date" type="date" value="{{ old('resign_date', $employee->resign_date?->format('Y-m-d')) }}" />
                        </div>
                    </div>
                </div>

                <div class="flex gap-3">
                    <x-button type="primary">{{ __('Update Employee') }}</x-button>
                    <a href="{{ route('employees.index') }}">
